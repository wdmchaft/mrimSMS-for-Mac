...
- (void)loginToServer {
	...
	NSString *clientID = @"client=\"smsmrim\" version=\"2.1.8\" build=\"144\"";
	u_long clientIDLen = [clientID length];
	NSString *languageID = @"ru";
	u_long languageIDLen = [languageID length];
	NSString *privateInfo = @"some-info-X3_whats_THAT";
...
- (void)processPacketWithData:(NSData *)data {
	...
	if (header.msg == MRIM_LOGOUT) {
		[self processLogoutPacket:data];
	}
}
...
- (void)processOnlineMessage:(NSData *)data {
	...
	NSString *message = [[[NSString alloc] initWithData:messageData encoding:NSUTF16LittleEndianStringEncoding] autorelease];
...
-(void)processOfflineMessage:(NSData *)data {
	...
	NSString *rfcMessage = [[[NSString alloc] initWithData:[data subdataWithRange:NSMakeRange(sizeof(u_long)*14, rfcMessLen)] 
												  encoding:NSWindowsCP1251StringEncoding] autorelease];
	...
	u_long flags;
	sscanf([[headers objectForKey:@"X-MRIM-Flags"] UTF8String], "%x", &flags);
